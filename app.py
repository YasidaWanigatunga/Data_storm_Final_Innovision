...
from generate_recommendations import (
    ...
    get_customer_data,FAISSRetriever,
    generate_insights,initialize_agent,query_agent
)
...
if trx_file and item_file:
    ...
    cluster_data, customer_data = perform_clustering(data)
    ...
    # Generate OpenAI insights for clusters
    ...
    if page == "Personalized product suggestions":
        ...
        if st.button("Search") and customer_code:
            ...
            if not customer_data.empty:
                ...
                st.error("No customer code found.")
        ...
        if st.button("Generate Promotion") and customer_code:
            customer_row = get_customer_data(customer_data, customer_code)
            if customer_row is not None:
                promotion = generate_personalized_promotion(customer_row)
        ...
        if st.button("Ask") and question:
            vectorstore = load_vectorstore("vectorstore.index")
            retriever = FAISSRetriever(vectorstore, data['combined_text'].tolist())
            agent = initialize_agent(data)
